...
from kubernetes.client import V1Node, V1Deployment, V1Service, V1ObjectMeta, V1DeploymentStatus, V1DeploymentCondition, V1ServiceSpec, V1ServicePort, V1DeploymentSpec
from api.server.util import gather_gpu_info, GPU, GraValBootstrapFailure, get_session, _fetch_devices, K8sOperator, settings
...
async def test_successful_gpu_gathering(mock_node, mock_deployment, mock_service, mock_devices):
    ...
    with patch('api.server.util.K8sOperator') as mock_operator, \
         patch('api.server.util._fetch_devices', return_value=mock_devices) as mock_fetch, \
         patch('api.server.util.get_session') as mock_get_session, \
         patch('api.server.util.GPU') as mock_gpu_class, \
...
async def test_missing_gpu_short_ref_label(mock_deployment, mock_service):
    ...
    with pytest.raises(GraValBootstrapFailure, match="Node does not have required gpu-short-ref label!"):
...
async def test_deployment_timeout(mock_node, mock_deployment, mock_service):
    ...
    with patch('api.server.util.K8sOperator') as mock_operator, \
         patch('api.server.util.settings') as mock_settings, \
         patch('time.time', side_effect=[0, 65]):  # Simulate timeout
        
        mock_settings.graval_bootstrap_timeout = 60
        mock_operator.return_value.watch_deployments.return_value = iter([mock_watch_event])
        
        with pytest.raises(GraValBootstrapFailure, match="GraVal bootstrap deployment not ready after"):
            await gather_gpu_info(
                server_id="server-123",
                validator="validator-456",
                node_object=mock_node,
...
async def test_watch_stream_exception(mock_node, mock_deployment, mock_service):
    ...
    with patch('api.server.util.K8sOperator') as mock_operator, \
        ...
        with pytest.raises(GraValBootstrapFailure, match="Error waiting for graval bootstrap deployment: Connection error"):
            await gather_gpu_info(
                server_id="server-123",
                validator="validator-456",
                node_object=mock_node,
...
async def test_device_fetch_failure(mock_node, mock_deployment, mock_service):
    ...
    with patch('api.server.util.K8sOperator') as mock_operator, \
         patch('api.server.util._fetch_devices', side_effect=Exception("Connection failed")) as mock_fetch, \
         patch('api.server.util.settings') as mock_settings:
        
        mock_settings.graval_bootstrap_timeout = 60
        mock_operator.return_value.watch_deployments.return_value = iter([mock_watch_event])
        
        with pytest.raises(GraValBootstrapFailure, match="Failed to fetch devices from GraVal bootstrap"):
            await gather_gpu_info(
                server_id="server-123",
                validator="validator-456",
                node_object=mock_node,
...
async def test_gpu_count_mismatch(mock_node, mock_deployment, mock_service):
    ...
    with patch('api.server.util.K8sOperator') as mock_operator, \
         patch('api.server.util._fetch_devices', return_value=mock_devices), \
         patch('api.server.util.settings') as mock_settings:
...
async def test_no_node_port_in_service(mock_node, mock_deployment):
    ...
    with patch('api.server.util.watch.Watch') as mock_watch, \
         patch('api.server.util.K8sOperator'), \
         patch('api.server.util._fetch_devices', side_effect=Exception("Connection failed")), \
         patch('api.server.util.settings') as mock_settings:
...
@pytest.mark.asyncio
async def test_default_namespace_fallback(mock_node, mock_service, mock_devices, mock_k8s_app_client):
...
async def test_zero_gpu_count(mock_deployment, mock_service):
    ...
    with patch('api.server.util.K8sOperator') as mock_operator, \
         patch('api.server.util._fetch_devices', return_value=[]), \
         patch('api.server.util.get_session') as mock_get_session, \
         patch('api.server.util.settings') as mock_settings:
...
async def test_empty_devices_response(mock_node, mock_deployment, mock_service):
    ...
    with patch('api.server.util.K8sOperator') as mock_operator, \
         patch('api.server.util._fetch_devices', return_value=None), \
         patch('api.server.util.settings') as mock_settings:
...
async def test_deployment_never_becomes_ready(mock_node, mock_deployment, mock_service):
    ...
    with patch('api.server.util.watch.Watch') as mock_watch, \
         patch('api.server.util.K8sOperator'), \
         patch('api.server.util.settings') as mock_settings, \
         patch('time.time', return_value=0):  # No timeout, just stream ends
        
        mock_settings.graval_bootstrap_timeout = 60
        mock_watch.return_value.stream.return_value = iter([mock_watch_event])
        
        with pytest.raises(GraValBootstrapFailure, match="GraVal bootstrap deployment never reached ready state"):
            await gather_gpu_info(
                server_id="server-123",
                validator="validator-456",
                node_object=mock_node,

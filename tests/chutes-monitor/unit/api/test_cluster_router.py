...
def test_register_cluster_success(mock_cluster_monitor, test_client, create_cluster_request_data):
    ...
    response = test_client.post("/test-cluster", json=request_data)
    
    assert response.status_code == 200
    mock_cluster_monitor.register_cluster.assert_called_once()


def test_register_cluster_failure(mock_cluster_monitor, test_client, create_cluster_request_data):
...
def test_unregister_cluster_success(mock_cluster_monitor, test_client):
    ...
    response = test_client.delete("/test-cluster")
...
def test_receive_heartbeat_success(mock_redis_client, test_client):
    ...
    response = test_client.post("/test-cluster/health", json=heartbeat_data)
...
def test_receive_heartbeat_unhealthy_cluster(mock_redis_client, test_client):
...
def test_receive_resource_update_success(mock_redis_client, test_client, test_resource_update_request):
    ...
    response = test_client.put("/test-cluster/resources", json=json_data)
